...
const BlogCategories = ({ name }) => {
    ...
    const getData = async () => {
        try {
            let response = await axios.get(`http://3.88.73.172:3001/v1/blogs?page=${currentPage}&limit=10&categoryId=${localStorage.getItem('categoryId')}`);
    ...
    const searchPage = async (number) => {
        try {
            setPage(number);
            let response = await axios.get(`http://3.88.73.172:3001/v1/blogs?page=${number}&limit=10&categoryId=${localStorage.getItem('categoryId')}`);
            setBlogs(response.data.data);
        } catch (error) {
            ToggleNotification('Error', error?.response?.data?.message);
        }
    ...
    const goToBlogDetails = (id, name) => {
        ...
        if(typeof name[0]?.props?.children[0] === 'string') {
            ...
            router.push(`/${name[0]?.props?.children[0].props.children[0].replace(/\s/g, "-").toLowerCase()}`);
        }
    };

    const goToBlogCategories = (id, name) => {
        localStorage.setItem('categoryId', id);
        router.push(`/blog/${name.replace(/\s/g, "-").toLowerCase()}`);
    };
